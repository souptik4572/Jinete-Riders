from django.http import JsonResponse
from .serializers import DriverSerializer
from .models import Driver, Token
from django.views.decorators.csrf import csrf_exempt
import json
import bcrypt
import jwt
from decouple import config
from datetime import datetime, timedelta
import os
import binascii
import pytz
from ..utils.mailer import send_password_reset_email

utc = pytz.UTC

ACCESS_SECRET_TOKEN = config('ACCESS_SECRET_TOKEN')
BCRYPT_SALT = int(config('BCRYPT_SALT'))


def are_passwords_matching(given_password, actual_password):
    return bcrypt.checkpw(given_password.encode('utf-8'), actual_password.encode('utf-8'))

# Create your views here.

@csrf_exempt
def get_profile_driver(request):
    if request.method == 'GET':
        try:
            passenger = Driver.objects.get(pk=request.session['user']['id'])
            return JsonResponse({
                'success': True,
                'message': 'Passenger profile data',
                'driver': DriverSerializer(passenger).data
            })
        except Driver.DoesNotExist:
            return JsonResponse({
                'success': False,
                'message': 'Passenger does not exist'
            })
        except Exception as e:
            return JsonResponse({
                'success': False,
                'message': str(e)
            }, 404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please send a GET request only for getting profile data'
        }, 404)

@csrf_exempt
def update_password(request):
    if request.method == 'PATCH':
        data = json.loads(request.body)
        try:
            token = data['token']
            driver_id = data['driver_id']
            password = data['password']
            driver = Driver.objects.get(pk=driver_id)
            password_reset_token = Token.objects.filter(
                driver=driver).first()
            if not password_reset_token:
                return JsonResponse({
                    'success': False,
                    'message': 'Invalid or expired password reset token'
                }, status=404)
            if utc.localize(datetime.now()) > password_reset_token.expiry_time:
                password_reset_token.delete()
                return JsonResponse({
                    'success': False,
                    'message': 'Expired password reset token'
                }, status=404)
            if not bcrypt.checkpw(token.encode('utf-8'), password_reset_token.token.encode('utf-8')):
                return JsonResponse({
                    'success': False,
                    'message': 'Invalid or expired password reset token'
                }, status=404)
            hashed_password = str(bcrypt.hashpw(
                password.encode('utf-8'), bcrypt.gensalt(BCRYPT_SALT))).replace("b'", "").replace("'", "")
            driver.password = hashed_password
            driver.save()
            password_reset_token.delete()
            return JsonResponse({
                'success': True
            }, status=201)
        except Driver.DoesNotExist:
            return JsonResponse({
                'success': False,
                'message': 'Driver with given id does not exist'
            }, status=404)
        except KeyError:
            return JsonResponse({
                'success': False,
                'message': 'Please provide all the fields'
            }, status=404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please only provide a PATCH for password update with neccessary data'
        }, status=404)


@csrf_exempt
def request_password_reset(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        try:
            email = data['email']
            driver = Driver.objects.filter(email=email).first()
            if not driver:
                return JsonResponse({
                    'success': False,
                    'message': 'Driver with given email does not exist'
                }, status=404)
            token = Token.objects.filter(driver_id=driver.id).filter()
            if token:
                token.delete()
            reset_token = binascii.b2a_hex(os.urandom(16))
            hash = str(bcrypt.hashpw(reset_token, bcrypt.gensalt(
                BCRYPT_SALT))).replace("b'", "").replace("'", "")
            token = Token.objects.create(driver=driver, token=hash)
            reset_data = {
                'token': str(reset_token).replace("b'", "").replace("'", ""),
                'id': DriverSerializer(driver).data['id'],
                'receiver': email,

            }
            send_password_reset_email(reset_data, True)
            return JsonResponse({
                'success': True,
            }, status=200)
        except KeyError:
            return JsonResponse({
                'success': False,
                'message': 'Email data missing'
            }, status=404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please only provide a POST request for resetting password with email'
        }, status=404)


@csrf_exempt
def edit_driver_data(request):
    if request.method == 'PATCH':
        data = json.loads(request.body)
        try:
            driver = Driver.objects.get(pk=request.session['user']['id'])
            print(driver.id)
            if 'name' in data:
                driver.name = data['name']
            if 'email' in data:
                driver.email = data['email']
            if 'phone' in data:
                driver.phone = data['phone']
            if 'city' in data:
                driver.city = data['city']
            if 'country' in data:
                driver.country = data['country']
            if 'profile_image' in data:
                driver.profile_image = data['profile_image']
            driver.save()
            return JsonResponse({
                'success': True,
                'message': 'Profile has been updated successfully',
                'driver': DriverSerializer(driver).data
            }, status=201)
        except Exception as e:
            return JsonResponse({
                'success': False,
                'message': str(e)
            }, status=404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please only provide a PATCH request for driver data updation'
        }, status=404)


@csrf_exempt
def login_driver(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        try:
            email = data['email']
            password = data['password']
            if not Driver.objects.filter(email=email).exists():
                return JsonResponse({
                    'success': False,
                    'message': 'Driver with given email does not exist'
                }, status=404)
            driver = Driver.objects.get(email=email)
            if not are_passwords_matching(password, driver.password):
                return JsonResponse({
                    'success': False,
                    'message': 'Passwords does not match'
                }, status=404)
            encoded_token = jwt.encode({
                'id': driver.id,
                'user_type': 'driver',
                'exp': datetime.now() + timedelta(days=1)
            }, ACCESS_SECRET_TOKEN, algorithm='HS512')
            return JsonResponse({
                'success': True,
                'message': 'Successfully logged in',
                'token': encoded_token
            }, status=200)
        except KeyError:
            return JsonResponse({
                'success': False,
                'message': 'Please provide both email and password for logging in'
            }, status=404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please only provide a POST request for login with necessary data'
        }, status=404)


@csrf_exempt
def register_driver(request):
    if request.method == 'PUT':
        data = json.loads(request.body)
        try:
            name = data['name']
            email = data['email']
            phone = data['phone']
            password = data['password']
            city = data['city']
            country = data['country']
            profile_image = data['profile_image']
            car_name = data['car_name']
            car_number = data['car_number']
            car_image = data['car_image']
            driving_license_no = 'driving_license_no'
            if Driver.objects.filter(email=email).exists():
                return JsonResponse({
                    'success': False,
                    'message': 'Driver already exists'
                }, status=404)
            hashed_password = str(bcrypt.hashpw(
                password.encode('utf-8'), bcrypt.gensalt(BCRYPT_SALT))).replace("b'", "").replace("'", "")
            try:
                driver = Driver.objects.create(
                    name=name, email=email, phone=phone, password=hashed_password, city=city, country=country, profile_image=profile_image, car_name=car_name, car_number=car_number, car_image=car_image, driving_license_no=driving_license_no)
                return JsonResponse({
                    'success': True,
                    'message': 'Succesfully created driver',
                    'driver': DriverSerializer(driver).data
                }, status=201)
            except Exception as e:
                return JsonResponse({
                    'success': False,
                    'message': str(e)
                }, status=404)
        except KeyError:
            return JsonResponse({
                'success': False,
                'message': 'Some data missing'
            }, status=404)
    else:
        return JsonResponse({
            'success': False,
            'message': 'Please only provide a PUT request with necessary data'
        }, status=404)
